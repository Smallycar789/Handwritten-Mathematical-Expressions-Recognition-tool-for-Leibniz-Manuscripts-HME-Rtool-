...
class ExpRateRecorder(Metric):
    def __init__(self, dist_sync_on_step=False):
        ...
        self.failed_preds = []  # Store (img_base, truth, pred)

    def update(self, indices_hat: List[List[int]], indices: List[List[int]], img_bases: List[str]):
        for img_base, pred, truth in zip(img_bases, indices_hat, indices):
            pred_str = vocab.indices2label(pred)
            truth_str = vocab.indices2label(truth)

            if pred_str == truth_str:
                self.rec += 1
            else:
                self.failed_preds.append((img_base, truth_str, pred_str))

            self.total_line += 1

    def compute(self) -> float:
        return self.rec / self.total_line

    def save_failed_preds(self, filepath="failed_predictions.txt"):
        with open(filepath, "w") as f:
            for img_base, truth, pred in self.failed_preds:
                f.write(f"Image: {img_base}\n")
