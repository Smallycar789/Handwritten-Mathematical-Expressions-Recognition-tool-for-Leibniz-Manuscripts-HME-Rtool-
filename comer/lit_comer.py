...
class LitCoMER(pl.LightningModule):
    def __init__(
        ...
        if ckpt_path is not None:
            ...
            if finetune is True:
                '''
                # Filter out mismatched output layer
                filtered = {
                    k: v for k, v in state_dict.items()
                    if not k.endswith("proj.weight") and not k.endswith("proj.bias") and not k.endswith("word_embed.0.weight") and not k.endswith("word_embed.0.bias")
                }
                missing, unexpected = self.load_state_dict(filtered, strict=False)'''
                print("-------------------------------Fine-tuning from Crohme -------------------------------")
            else:
                
                print("-------------------------------Full checkpoint load-------------------------------")
    ...
    def validation_step(self, batch: Batch, _):
        ...
        self.exprate_recorder([h.seq for h in hyps], batch.indices, batch.img_bases)
        self.log(
            "val_ExpRate",
    ...
    def test_step(self, batch: Batch, _):
        ...
        img_bases = batch.img_bases  # Names of cropped images
        preds = [h.seq for h in hyps]
        print("=====================The prediction is :", preds, "=======================")
        #truths = batch.indices
    
        #self.exprate_recorder(preds, truths, img_bases)
        return img_bases, [vocab.indices2label(h.seq) for h in hyps]
    '''
    def test_step(self, batch: Batch, _):
        hyps = self.approximate_joint_search(batch.imgs, batch.mask)
    ...
    def test_epoch_end(self, test_outputs) -> None:
        ...
        self.exprate_recorder.save_failed_preds("failed_predictions.txt")
