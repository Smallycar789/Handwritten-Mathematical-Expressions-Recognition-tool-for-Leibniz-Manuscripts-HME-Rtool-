...
import torch
from comer.datamodule.dataset import LHDataset
...
def extract_data(archive: ZipFile, dir_name: str) -> Data:
    ...
    """
    with archive.open(f"data/{dir_name}/annotations.txt", "r") as f:
...
class LHDatamodule(pl.LightningDataModule):
    def __init__(
        self,
        zipfile_path: str = f"{os.path.dirname(os.path.realpath(__file__))}/../../"+f"{zipfile_path}",
